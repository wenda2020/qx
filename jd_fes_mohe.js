...
async function main() {
    ...
    $.mainTime = 0;
    do {
        let task = await getApi('getMyTask');
        $.myTasks = task.myTasks || [];
        $.code = task.code;
        for (let k = 0; k < $.myTasks.length; k++) {
            $.taskType = $.myTasks[k].taskType;
            $.taskId = $.myTasks[k].taskId;
            if ($.myTasks[k].taskItem != null) {
                if ($.taskId != 138) {
                    $.runTime = parseInt($.myTasks[k].itemCount) - parseInt($.myTasks[k].finishCount);
                    if ($.mainTime == 0) {
                        $.maxtime.push($.runTime);
                    }
                    $.itemId = $.myTasks[k].taskItem.itemId;
                    if ($.runTime > 0) {
                        if ($.taskId == 139) {
                            await doTask(0);
                        }
        ...
        if ($.mainTime == 0) {
            $.tie = Math.max.apply(null,$.maxtime);
            console.log(`共需要进行 ${$.tie} 次任务`);
        }
        $.mainTime++;
        console.log(`完成第 ${$.mainTime} 次任务`)
    } while ($.mainTime < $.tie);
    let myrewards = await getApi('getMyRewards');
    console.log(`当前心动值: ${myrewards.totalPoints}`)
    if (Candraw == 1) {
        console.log(`开始抽奖`);
        for (let j = 0; j < Candrawtime; j++) {
            let draw = await getApi('draw');
            if (draw.rewardType == 2) {
                console.log(`获得优惠券: ${draw.couponName}`)
            } else if (draw.rewardType == 4) {
                console.log(`获得 ${draw.rewardNum} 心动值`)
            }
            //console.log(draw)
            await $.wait(1000)
        }
...
async function doTask(isCommon) {
    let options = {
        url: `https://jdjoy.jd.com/module/freshgoods/doTask`,
        body: isCommon ? `code=${$.code}&taskType=${$.taskType}&taskId=${$.taskId}&eid=${$.eid}&fp=${$.fp}&itemId=${$.itemId}` : `code=${$.code}&taskType=${$.taskType}&taskId=${$.taskId}&eid=${$.eid}&fp=${$.fp}`,
        headers: {
            "Host":"jdjoy.jd.com",
