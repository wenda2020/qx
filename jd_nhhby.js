          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode == 0) {
                if (data.data.result.hasOwnProperty(hbInfo)) {
                    ...
                    message += `\n【京东账号${$.index}】${$.nickName || $.UserName}\n获得年货红包 ${data.data.result.hbInfo.discount} 元`
                }
            } else if (data.data.bizCode == -1006) {
              console.log(data.data.bizMsg)
            } else if (data.data.bizCode == -9001) {
              console.log(data.data.bizMsg)
            }
          }
        }
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=publicUseApi&body=${escape(JSON.stringify(body))}&t=${Date.now()}&client=wh5&clientVersion=1.0.0&networkType=&ext={"prstate":"0"}`,
    headers: {
      "Accept": "*/*",
