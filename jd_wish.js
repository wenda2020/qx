...
async function jd_wish() {
  try {
    ...
    $.canLottery = true
      if (Candraw == 1) {
          for (let j = 0; j < Candrawtimes && $.canLottery; j++) {
                        ...
                        if (type) {
                            ...
                            for (let key of Object.keys(data.data.result.taskVos).reverse()) {
                                ...
                                if (vo.status !== 2 && vo.status !== 0) {
                                    if (vo.taskType === 13 || vo.taskType === 12) {
                                        ...
                                        for (let key of Object.keys(vo.browseShopVo)) {
                                            ...
                                            if (browseShopVo.status !== 2) {
                                                ...
                                                await harmony_collectScore({"appId":appId,"taskToken":browseShopVo.taskToken,"taskId":vo.taskId,"actionType":"1"})
                                                if ($.complete) break;
                                            }
                                        }
                                    ...
                                    } else if (vo.taskType === 9 || vo.taskType === 26 || vo.taskType === 3) {
                                        $.complete = false;
                                        for (let key of Object.keys(vo.shoppingActivityVos)) {
                                            ...
                                            if (shoppingActivityVos.status !== 2) {
                                                ...
                                                if (vo.taskType === 9 || vo.taskType === 3) {
                            ...
                            for (let hotkey of Object.keys(data.data.result.hotTaskVos).reverse()) {
                                let hotvo = data.data.result.hotTaskVos[hotkey]
                                if (hotvo.status !== 2 && hotvo.status !== 0) {
                                    if (hotvo.taskType === 12) {
                                        console.log(`完成【${hotvo.taskName}】`)
                                        await harmony_collectScore({"appId":appId,"taskToken":hotvo.simpleRecordInfoVo.taskToken,"taskId":hotvo.taskId,"actionType":"0"}, hotvo.taskType)
                                    }
                                } else {
                                    console.log(`【${hotvo.taskName}】已完成\n`)
                                }
                            }
                        }
                    }
