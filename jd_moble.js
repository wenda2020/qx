...
async function jd_wish() {
    try {
        let taskListdata = await takePost(`{"apiMapping":"/api/task/getTaskList"}`);//获取分任务列表
        ...
        for (let i = 0; i < $.tasklist.length; i++) {
            ...
            $.taskItemType1 = $.tasklist[i].taskItemType;
            $.hasNum = parseInt($.tasklist[i].totalNum) - parseInt($.tasklist[i].finishNum);//计算还需做多少次
            await $.wait(1000)
            for (let m = 0; m < $.hasNum; m++) {
                let tasklastdata = await takePost(`{"apiMapping":"/api/task/getTaskList"}`);
                $.lastlist = tasklastdata.taskList || [];
                for (let ite of $.lastlist) {
                    if (ite.taskItemType == $.taskItemType1) {
